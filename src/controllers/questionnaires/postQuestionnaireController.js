...
async function postQuestionnaireController(req, res){
    ...
    try{
        ...
        if(questionnaire){
            ...
            requestLogger.info('Response POST /questionnaire', {requestID: req.id, status: 201});
            requestLogger.debug('Quiestionnaire response POST /questionnaire', {requestID: req.id, questionnaire: questionnaire});
        }
        else{
            res.status(400).json({error: 'Bad Request'});
            requestLogger.info('Response POST /questionnaire', {requestID: req.id, status: 400});
            requestLogger.debug('Invalid body POST /questionnaire', {requestID: req.id, body: body});
        }
    }
    catch(err){
        res.status(500).json({error: 'Server Error'});
        requestLogger.error('Error POST /questionnaire', {requestID: req.id, status: 500, body: body, error: String(err)});
    }
}
